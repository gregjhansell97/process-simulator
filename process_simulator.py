...
def FCFS(process_list):
    ...
    while not done(process_list):
        ...
        if(wait > 0):
            ...
            wait -= 1
            continue
        #handling running process
        if running_process is None: #need to set a running process
            ...
            if(running_process.current_burst <= 0): #running process has finished bursts
                if running_process.restart():
                    ...
                    print("time " + str(clock + 4) + "ms: Process " + str(running_process.id) + " switching out of CPU; will block on I/O until time " + str(clock + running_process.io_burst) + " " + str(ready_q))
                    wait = 3
        ...
        while i < len(blocked): #looping through blocks
            if blocked[i].io_burst <= 0: #if I/O is done
                ...
                print("time " + str(clock) + "ms: Process " + str(change_process.id) + " completed I/O; added to ready queue " + str(ready_q))
                continue
